...
void PclTestCore::point_cb(const sensor_msgs::PointCloud2ConstPtr &in_cloud_ptr)
{
    ...
    pcl::fromROSMsg(*in_cloud_ptr, *current_pc_ptr);//将pointcloud2转换成PointXYZI

    clip_above(CLIP_HEIGHT, current_pc_ptr, cliped_pc_ptr);//裁剪高处点云
    pcl::PointCloud<pcl::PointXYZI>::Ptr remove_close(new pcl::PointCloud<pcl::PointXYZI>); 

    remove_close_pt(MIN_DISTANCE, cliped_pc_ptr, remove_close);  //移除近处点云
